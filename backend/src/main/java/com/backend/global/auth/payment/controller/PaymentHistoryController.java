...
public class PaymentHistoryController {
    ...
    @GetMapping("/{memberId}")
    public GenericResponse<List<BlossomTransactionDto>> getTransactionHistory(@PathVariable Long memberId) {
        List<BlossomTransaction> blossomTransactions = transactionRepository.findByMemberId(memberId);
        List<BlossomTransactionDto> blossomTransactionDto = blossomTransactions.stream().map(tx -> new BlossomTransactionDto(
                tx.getId(),
                tx.getMember().getId(),
                tx.getTotalAmount(),
                tx.getTossOrderId(),
        ...
        return GenericResponse.ok(blossomTransactionDto, "거래 내역 조회에 성공하였습니다.");
    }
}
