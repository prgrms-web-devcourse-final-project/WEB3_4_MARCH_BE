...
export const ProfileSetupActivity = ({
  ...
  const onConfirmKeywordSetup = async (keywordIds: number[]) => {
    ...
    if (!profile || profile.role === "ROLE_TEMP_USER") {
      await apiClient.member.register({
        registerDTO: {
          member: {
      ...
      if (!profile.id) {
        alert("프로필의 아이디가 없습니다");
        return;
      }

      await apiClient.member.modifyMemberInfo({
        id: profile.id,
        modifyMemberDto: {
          keepImageIds: [],
          newImages: images,
  ...
  return (
    <AppScreenLayout noBottomBar title="프로필 설정">
      ...
      {page === "keyword" && (
        <ProfileKeywordView
          onComplete={onConfirmKeywordSetup}
          defaultKeywords={profile ? (profile.keywords ?? []) : []}
        />
      )}
    </AppScreenLayout>
  );
